// Run with: node test/climb-scenario-test.js  (from server/ directory)
import { getClimbYRef, getDist, getTime, getFuel } from '../src/lib/climb-calc.js';
import * as pa28151Climb from '../src/data/pa28-151-climb-data.js';
import * as pa28161Climb from '../src/data/pa28-161-climb-data.js';
import * as pa28181Climb from '../src/data/pa28-181-climb-data.js';
...
for (const s of SCENARIOS) {
    ...
    for (const [label, computed, expected] of [
        ['Time (min)', netTime, s.expected.time],
        ['Dist (nm) ', netDist, s.expected.dist],
        ['Fuel (gal)', netFuel, s.expected.fuel],
    ]) {
        const line = fmtField(label, computed, expected);
        console.log(line);
        if (expected === null)              tbd++;
