...
const CALIBRATIONS = {
    ...
    'pa28-161': {
        ...
        // PA28-161TakeoffPerfChart.png (Figure 5-9) — 50 ft obstacle distance.
        // Image: 1963×1263 px. Three-pane chart: oat (left), weight (middle),
        // wind (right). All pixel anchors are INITIAL ESTIMATES from image
        // inspection — tune using the trace overlay page.
        // yRef scale 0-80 assumed (same as cruise/engine); confirm via overlay.
        // Weight axis runs RIGHT-TO-LEFT (2400 lbs at left, 1700 lbs at right) —
        // refPoints reflect that inversion: higher data.x (2400) maps to lower
        // pixel.x. Wind axis runs LEFT (0 kt) to RIGHT (15 kts).
        // PA28-161TakeoffPerfChart.png (Figure 5-9) — distance over 50 ft obstacle.
        // Image: 1963×1263 px. Y-axis in FEET (0–4000 ft), confirmed by 8 full-width
        // row-detected gridlines at 122.6 px/500 ft pitch (MINSTR=0.25 scan).
        // Y=185.6 px = 4000 ft (top, start of data grid), Y≈1166.2 px = 0 ft (extrapolated).
        // OAT panel: left x=286.9 (−40°C), right x=793.1 (+40°C), skew negligible (+0.0001).
        // Weight axis inverted: data x=2400 (max) on LEFT (px x≈793), data x=1700 on RIGHT
        // (px x≈1305–1306). Right border slope −0.00143.
        // Wind panel: left x=1333.5→1330.8 (slope −0.00274), right x=1516.8→1515.8 (slope −0.00099).
        // Y axis is a reference scale (0–80) matching the chart's printed gridlines.
        // Pixel anchors measured from chart; yRef1/yRef2 data values use the same 0–80 scale.
        takeoff50: {
        ...
        // Image: 1968×1239 px. Top outer frame y=131.5 is chart border; labeled data
        // starts at y=498.2. Y confirmed by 6 right-margin ticks (x=1675–1690): pitch
        // 125.2 px/500 ft. OAT panel: left x=329.5→330.3 (slope +0.00125), right
        // x=803.1→802.9 (negligible). Weight/wind boundary x=1487.9→1488.0.
        // Wind panel right x=1673.6→1671.7 (slope −0.00299).
        // Y axis is a reference scale (0–80) matching the chart's printed gridlines.
        takeoffRoll: {
            ...
            panels: {
                oat: {
                    ...
                    yRange: [0, 60],
                    refPoints: [
                        { data: { x: -40, y:  0 }, px: { x:  330.3, y: 1110.0 } },
                        { data: { x:  40, y:  0 }, px: { x:  815, y: 1110.0 } },
                        { data: { x: -40, y: 60 }, px: { x:  329.5, y:  382 } },
                        { data: { x:  40, y: 60 }, px: { x:  815, y:  382 } },
                    ],
                },
                weight: {
                    xRange: [1600, 2440],
                    yRange: [0, 60],
                    refPoints: [
                        { data: { x: 2400, y:  0 }, px: { x:  937, y: 1114.0 } },
                        { data: { x: 1600, y:  0 }, px: { x: 1427, y: 1114.0 } },
                        { data: { x: 2400, y: 60 }, px: { x:  937, y:  382 } },
                        { data: { x: 1600, y: 60 }, px: { x: 1427, y:  381 } },
                    ],
                },
                wind: {
                    xRange: [0, 15],
