...
export function drawLattice(svg, transform, panel, xStep, yStep) {
    ...
    g.classList.add('lattice-group');
    for (let x = x0; x <= x1 + 1e-9; x += xStep) {
        const el = document.createElementNS(SVG_NS, 'polyline');
        el.setAttribute('points', [{ x, y: y0 }, { x, y: y1 }].map(p => {
            const q = dataToPx(transform, p);
            return `${q.x.toFixed(1)},${q.y.toFixed(1)}`;
        }).join(' '));
        el.setAttribute('fill', 'none'); el.setAttribute('stroke', '#dc2626');
        el.setAttribute('stroke-width', 1); el.setAttribute('vector-effect', 'non-scaling-stroke');
        g.appendChild(el);
    }
    for (let y = y0; y <= y1 + 1e-9; y += yStep) {
        const el = document.createElementNS(SVG_NS, 'polyline');
        el.setAttribute('points', [{ x: x0, y }, { x: x1, y }].map(p => {
            const q = dataToPx(transform, p);
            return `${q.x.toFixed(1)},${q.y.toFixed(1)}`;
        }).join(' '));
        el.setAttribute('fill', 'none'); el.setAttribute('stroke', '#dc2626');
...
export function drawTraces(svg, transforms, traces, color = '#0f766e') {
    ...
    for (const trace of traces) {
        ...
        for (const dot of trace.dots) {
            addDot(g, transforms[dot.panel], dot, color);
        }
    }
    svg.appendChild(g);
}
